...
const REVEAL_AT_Y = 100;     // show when near top
const HIDE_DELTA = 24;       // hide after small downward scroll
...
export default function TopBar() {
  ...
  // Reset lastY on route change so the bar doesn't get stuck hidden
  useEffect(() => {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (y < REVEAL_AT_Y) {
        setHidden(false); // near top: reveal
      } else if (goingDown && y - lastY.current > HIDE_DELTA) {
        setHidden(true);  // small downward bump: hide
      }
      lastY.current = y;
    };
