...
export type FinderFilters = {
  ...
  age: Age;
  minChapters: number;              // 0 = any
  genres: number[];
  genresMode: MatchMode;
  tagsInclude: number[];
  tagsMode: MatchMode;
  tagsExclude: number[];
  folderInclude: number | null;     // null = no filter
  folderExclude: number | null;     // null = no filter
  sortBy: SortBy;
  sortOrder: SortOrder;
};
...
export async function findNovels(
  ...
  if (filters.releaseStatus !== "all") {
    where.push("n.release_status = ?");
    params.push(filters.releaseStatus);
  }

  // age
  const ageToSeconds: Record<Age, number> = {
    all: 0, "24h": 24*3600, "7d": 7*24*3600, "30d": 30*24*3600,
    "6mo": 183*24*3600, "12mo": 365*24*3600
  };
  const ageSec = ageToSeconds[filters.age] || 0;
  if (ageSec > 0) {
    where.push("n.created_at >= (unixepoch() - ?)");
    params.push(ageSec);
  }

  // min chapters
  if (filters.minChapters > 0) {
    where.push("(s.chapter_count >= ?)");
    params.push(filters.minChapters);
  }

  // genres include (AND/OR)
  ...
  const sql = `
    SELECT
      ...
      -- small correlated lists for the page
      IFNULL((
